...
if True:
    ...
    for name, graph in GRAPHS.items():
        for eW in edge_weights_for_LTM:
            ...
            handler = CPC.CpcHandler(graph, cores=CORES, seed_function=seed_function, sweeps=SWEEPS, model='LTM')
            ...
            try:
                ...
                for node in G.nodes():
                    degree_cpc_tryout.append((G.degree(node), G.nodes[node]['CPC'], name, eW, len(G.nodes()), spreadingDensity))
                ...
                print(f"[SKIP] Error for graph '{name}' at eW={eW}: {e}")
                traceback.print_exc()
                ...
                try:
                    ...
                    print(f"[ERROR] Diagnostic check failed for '{name}' at eW={eW}: {diag_e}")
        ...
        df = pd.DataFrame(degree_cpc_tryout, columns=['degree', 'CPC', 'name', 'eW', 'Number_of_nodes', 'spreadingDensity'])
    ...
    for name, graph in GRAPHS.items():
        for mean_eW in edge_weights_for_LTM:
            ...
            try:
                ...
                for node in G.nodes():
                    degree_cpc_tryout.append((G.degree(node), G.nodes[node]['CPC'], name, mean_eW, len(G.nodes()), spreadingDensity))
        ...
        df = pd.DataFrame(degree_cpc_tryout, columns=['degree', 'CPC', 'name', 'mean_eW', 'Number_of_nodes', 'spreadingDensity'])

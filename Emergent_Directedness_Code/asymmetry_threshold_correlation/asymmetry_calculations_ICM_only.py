...
for name, graph in GRAPHS.items():
    for eP in edge_probabilities:
        ...
        handler = CPC.CpcHandler(graph, cores=CORES, seed_function=seed_function, sweeps=SWEEPS, model='ICM', random_seed_np=SEED_NP)
        ...
        try:
            ...
            eigenvector = nx.eigenvector_centrality(G, max_iter=1000)  # Adjust max_iter if needed
        except nx.NetworkXException as e:
            print(f"Centrality computation failed for {name} at eP={eP}: {e}")
            betweenness = closeness = eigenvector = {n: 0 for n in G.nodes()}  # Fallback

        for node in G.nodes():
            degree_cpc_tryout.append((G.degree(node), betweenness[node], closeness[node], eigenvector[node], G.nodes[node]['CPC'], name, eP, len(G.nodes()), spreadingDensity))
    ...
    df = pd.DataFrame(degree_cpc_tryout, columns=['degree', 'betweenness', 'closeness', 'eigenvector', 'CPC', 'name', 'eP', 'Number_of_nodes', 'spreadingDensity'])
